...
func Init() error {
	if err := initMcpToolsToAll(config.Conf.MCPPath.Bangumi); err != nil {
...
func initMcpToolsToAll(path string) error {
	// 父级超时上下文
...
func initMcpToolsToFiltered(path string) error {
	// 父级超时上下文
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func initSSEMcpToolsToAll(path string) error {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
func initSSEMcpToolsToFiltered(path string) error {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
